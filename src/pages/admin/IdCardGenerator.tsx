...
import IdCardTemplate, { StudentData, CardTheme } from '@/components/id-card/IdCardTemplate';
import { useSchool } from '../../context/SchoolContext';

export default function IdCardGenerator() {
  const { students: contextStudents } = useSchool();
  const [theme, setTheme] = useState<CardTheme>('blue');
  const [activeTab, setActiveTab] = useState<'select' | 'preview' | 'batch'>('select');
  ...
  const classes = useMemo(() => {
    return ['Nursery', 'LKG', 'UKG', 'Class 1', 'Class 2', 'Class 3', 'Class 4', 'Class 5', 'Class 6', 'Class 7', 'Class 8', 'Class 9', 'Class 10', 'Class 11', 'Class 12'];
  }, []);

  // Filter students based on search and class dropdown
  ...
  }, [contextStudents, searchTerm, classFilter]);

  const selectedStudents = contextStudents.filter(s => selectedIds.has(s.id));
  ...
  const downloadBulkA4PDF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pageCount; i++) {
        ...
        if (frontEl) {
          const frontImg = await htmlToImage.toPng(frontEl, { pixelRatio: 2, style: { transform: 'none' } });
        ...
        if (backEl) {
          const backImg = await htmlToImage.toPng(backEl, { pixelRatio: 2, style: { transform: 'none' } });
          pdf.addPage();
    ...
    <div className="space-y-6 animate-fade-in print:bg-white print:m-0 print:p-0">
      ...
      <div className="print:hidden space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">ID Card Studio</h1>
            <p className="text-slate-500 text-sm mt-1">Design, generate and print premium student PVC cards.</p>
          </div>
          {selectedIds.size > 0 && (
        ...
        <div className="flex gap-2 border-b border-slate-200">
          <button 
            ...
            className={cn("px-4 py-3 text-sm font-semibold border-b-2 transition-colors", activeTab === 'select' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800')}
          >
            1. Select Students
          </button>
          <button 
            ...
            className={cn("px-4 py-3 text-sm font-semibold border-b-2 transition-colors disabled:opacity-40 disabled:cursor-not-allowed", activeTab === 'preview' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800')}
          >
            2. Design & Preview
          </button>
          <button 
            ...
            className={cn("px-4 py-3 text-sm font-semibold border-b-2 transition-colors disabled:opacity-40 disabled:cursor-not-allowed", activeTab === 'batch' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800')}
          >
            3. Batch Output
          </button>
        </div>
        ...
        {activeTab === 'select' && (
          <div className="bg-white rounded-[2rem] p-6 border border-slate-200 shadow-sm overflow-hidden">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-100">
                    ...
                    <th className="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Adm ID</th>
                    <th className="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Class/Sec</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredStudents.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-slate-500">No students found matching your criteria.</td>
                    </tr>
                  ) : (
                    filteredStudents.map(student => (
                      <tr key={student.id} className={cn("hover:bg-slate-50/50 transition-colors", selectedIds.has(student.id) ? 'bg-indigo-50/30' : '')}>
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-3">
                            <img src={student.photoUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${student.name}`} alt={student.name} className="w-8 h-8 rounded-full border border-slate-200" />
                            <span className="font-semibold text-sm text-slate-700">{student.name}</span>
                          </div>
                        </td>
                        <td className="py-3 px-4 text-sm text-slate-600 font-mono">{student.id}</td>
                        <td className="py-3 px-4 text-sm text-slate-600">{student.class} - {student.section}</td>
                        <td className="py-3 px-4">
                          <span className={cn("px-2.5 py-1 text-xs font-semibold rounded-md", student.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700')}>
                            {student.status}
                          </span>
                        </td>
                      </tr>
            ...
            <div className="mt-4 flex justify-between items-center text-sm text-slate-500">
              ...
              <p className="font-semibold text-indigo-600">{selectedIds.size} selected for printing</p>
            </div>
          </div>
        )}
          ...
          <div className="grid lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm overflow-hidden">
                <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><Palette className="w-4 h-4 text-indigo-600" /> Select Theme</h3>
                  ...
                  {templates.map(t => (
                    <button 
                      ...
                      className={cn(
                        ...
                        theme === t.id ? "border-indigo-600 bg-indigo-50 text-indigo-900 shadow-sm" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
                      )}
                    >
                      <div className={cn("w-5 h-5 rounded-full shadow-inner", t.color)}></div>
                      {t.name}
                    </button>
                  ))}
                </div>
              ...
              <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm overflow-hidden">
                <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><LayoutGrid className="w-4 h-4 text-indigo-600"/> Face Preview</h3>
                <div className="flex bg-slate-100 p-1 rounded-xl">
                  <button 
                    onClick={() => setPreviewSide('front')}
                    className={cn("flex-1 py-1.5 text-sm font-semibold rounded-lg transition-colors", previewSide === 'front' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500')}
                  >
                    Front
                  </button>
                  <button 
                    onClick={() => setPreviewSide('back')}
                    className={cn("flex-1 py-1.5 text-sm font-semibold rounded-lg transition-colors", previewSide === 'back' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500')}
                  >
                    Back
                  </button>
                </div>
            ...
            <div className="lg:col-span-3 bg-slate-50 rounded-[2rem] p-8 border border-slate-200 shadow-inner flex flex-col items-center justify-center min-h-[500px]">
               <div className="mb-6 text-center">
                 <h2 className="text-xl font-bold text-slate-800">Live Card Layout</h2>
                 <p className="text-sm text-slate-500">Previewing 1 of {selectedStudents.length} selected cards</p>
               </div>
               ...
               <div className="shadow-2xl rounded-[10px] overflow-hidden transform hover:scale-105 transition-transform duration-300">
                  <div id={`card-front-${selectedStudents[0].id}`}>
                    <IdCardTemplate student={selectedStudents[0]} theme={theme} side={previewSide} />
                  </div>
               </div>
               ...
               <div className="mt-8 flex gap-4">
                 ...
                 >
                   <FileDown className="w-4 h-4" /> {isGenerating ? 'Generating...' : 'Download This PDF'}
                 </button>
               </div>
            </div>
          </div>
        ...
        {activeTab === 'batch' && selectedStudents.length > 0 && (
           <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
             <div className="p-6 border-b border-slate-200 bg-slate-50/50">
               <h3 className="font-bold text-lg text-slate-800">Batch Processing Ready</h3>
               <p className="text-sm text-slate-600">{selectedStudents.length} student records selected for printing.</p>
             </div>
             <div className="p-6 grid sm:grid-cols-2 gap-6">
                <div className="border border-slate-200 rounded-2xl p-6 bg-white flex flex-col items-center text-center hover:border-indigo-300 hover:shadow-sm transition-all group">
                   ...
                   <h4 className="font-bold text-slate-800 mb-2">Print A4 Layouts</h4>
                   <p className="text-sm text-slate-500 mb-6">Generates an A4 portrait grid with 9 cards per page (3x3), properly aligned for double sided printing.</p>
                   <button 
                     onClick={handlePrintBatch}
                ...
                <div className="border border-slate-200 rounded-2xl p-6 bg-white flex flex-col items-center text-center hover:border-indigo-300 hover:shadow-sm transition-all group">
                   ...
                   <h4 className="font-bold text-slate-800 mb-2">Download Bulk PDF</h4>
                   <p className="text-sm text-slate-500 mb-6">Generates an A4 portrait PDF file containing 9 cards per page (front and back), ready for direct printing.</p>
                   <button 
                   ...
                   >
                     {isBulkDownloading ? 'Generating PDF...' : 'Download Bulk PDF'}
                   </button>
                </div>
             </div>
           </div>
      ...
      <div id="preview-print-zone-container" className="fixed -top-[20000px] left-0 print:static preview-print-zone bg-white pointer-events-none print:pointer-events-auto print:flex flex-col gap-8 w-fit shrink-0 opacity-100">
         <style>{`
           @media print {
         ...
         <div className="print-content" ref={printGridRef}>
           {Array.from({ length: Math.ceil(selectedStudents.length / 9) }).map((_, pageIndex) => {
             const pageStudents = selectedStudents.slice(pageIndex * 9, (pageIndex + 1) * 9);
             return (
               <div key={`page-${pageIndex}`}>
                 ...
                 >
                   <div className="card-grid">
                     {pageStudents.map((s) => (
                       <div key={`front-${s.id}`} id={`card-front-${s.id}-print`} className="relative p-0 bg-white inline-block">
                         <div className="absolute -inset-[1mm] border-[0.5mm] border-dashed border-gray-300 pointer-events-none" />
                         <IdCardTemplate student={s} theme={theme} side="front" />
                       </div>
                     ))}
                   </div>
                 </div>
                 ...
                 >
                   <div className="card-grid">
                     {Array.from({ length: 9 }).map((_, i) => {
                       ...
                       return (
                         <div key={`back-${targetStudent?.id || i}`} id={targetStudent ? `card-back-${targetStudent.id}-print` : undefined} className="relative bg-white inline-block">
                           {targetStudent ? (
                             <>
                               <div className="absolute -inset-[1mm] border-[0.5mm] border-dashed border-gray-300 pointer-events-none" />
                               <IdCardTemplate student={targetStudent} theme={theme} side="back" />
                             </>
